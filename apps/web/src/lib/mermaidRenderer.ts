...
async function renderUncachedMermaidSvg(source: string, theme: MermaidTheme): Promise<string> {
...
export function renderMermaidSvg(source: string, theme: MermaidTheme): Promise<string> {
  const cached = mermaidSvgCache.get(mermaidCacheKey(source, theme));
  if (cached !== null) return Promise.resolve(cloneMermaidSvg(cached));

  const result = mermaidRenderQueue.then(
    () => renderUncachedMermaidSvg(source, theme),
    () => renderUncachedMermaidSvg(source, theme),
  );
